...
<div class="flex flex-col items-center space-x-4">
    <div class="h-24 w-24 sm:h-32 sm:w-32 xl:h-40 xl:w-40 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
        @if ($avatarUrl)
            <img src="{{ $avatarUrl }}" alt="{{ $name }}" class="h-full w-full object-cover">
        @else
            <span class="text-2xl sm:text-3xl xl:text-4xl font-bold">{{ collect(explode(' ', $name))->map(fn($n) => $n[0])->join('') }}</span>
        @endif
    </div>
